    ...
    public class BlogPostOperations
    {
        ...
        public BlogIndexViewModel GetBlogIndexViewModel()
        {
            List<BlogPost> blogPosts = _unitOfWork.BlogPostSchoolRepository
                .Get(null, b => b.OrderByDescending(d => d.DateTime), "Author,BlogPostReactions").ToList();
            if (!blogPosts.Any())
            {
                return new BlogIndexViewModel();
            }
        ...
        public void update(int id,BlogPost blogPost)
        {
            try
            {
                ...
                post.ImageUrl = blogPost.ImageUrl;
                post.DateTime = blogPost.DateTime;
                post.Content = blogPost.Content;
                this._unitOfWork.BlogPostSchoolRepository.Update(post);
                this._unitOfWork.Save();
            }
            catch (Exception e)
            {
